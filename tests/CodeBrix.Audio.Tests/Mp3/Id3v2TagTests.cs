...
public class Id3v2TagTests
{
    ...
    public void ReadTagReturnsNullForNonId3DataAndRestoresPosition()
    {
        ...
        using (var stream = new MemoryStream(bytes))
        {
            ...
            Assert.Null(tag);
            Assert.Equal(start, stream.Position);
        }
    }
    ...
    public void ReadTagDoesNotThrowForNonId3Data()
    {
        ...
        using (var stream = new MemoryStream(bytes))
        {
            Assert.Null(Record.Exception(() => Id3v2Tag.ReadTag(stream)));
        }
    }
    ...
    public void ReadTagParsesHeaderOnlyTag()
    {
        ...
        using (var stream = new MemoryStream(data))
        {
            ...
            Assert.NotNull(tag);
            Assert.Equal(10, tag.RawData.Length);
            Assert.Equal(data, tag.RawData);
    ...
    public void ReadTagReadsPayloadBytes()
    {
        ...
        using (var stream = new MemoryStream(data))
        {
            ...
            Assert.NotNull(tag);
            Assert.Equal(10 + payload.Length, tag.RawData.Length);
            Assert.Equal(data, tag.RawData);
            Assert.Equal(start + data.Length, stream.Position);
        }
    }
    ...
    public void ReadTagRespectsStartOffset()
    {
        ...
        using (var stream = new MemoryStream(buffer))
        {
            ...
            Assert.Equal(tag, parsed.RawData);
            Assert.Equal(prefix.Length + tag.Length, stream.Position);
        }
    }
    ...
    public void ReadTagReadsFooterWhenFlagSet()
    {
        ...
        using (var stream = new MemoryStream(data))
        {
            ...
            Assert.NotNull(tag);
            Assert.Equal(data.Length, tag.RawData.Length);
            Assert.Equal(data, tag.RawData);
        }
    }
    ...
    public void ReadTagWithExtendedHeaderShouldNotConsumeBytesPastTagBoundary()
    {
        ...
        using (var stream = new MemoryStream(data))
        {
            ...
            Assert.NotNull(parsed);
            Assert.Equal(tag.Length, stream.Position);
        }
    }
    ...
    public void CreateWithNoTagsCreatesHeaderOnlyTag()
    {
        ...
        Assert.NotNull(tag);
        Assert.Equal(10, tag.RawData.Length);
        Assert.Equal("ID3", Encoding.ASCII.GetString(tag.RawData, 0, 3));
    }
    ...
    public void CreateCreatesTagContainingRequestedFrames()
    {
        ...
        Assert.NotNull(tag);
        Assert.Contains("TIT2", rawText);
        Assert.Contains("TPE1", rawText);
    }
