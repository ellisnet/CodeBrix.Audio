...
public class Mp3FrameDecompressorDimRoutingTests
{
    ...
    public void SpanOverloadRoutesThroughLegacyByteArrayOverload()
    {
        ...
        Assert.Equal(1, impl.DecompressCallsReceived);
        Assert.Equal(0, impl.LastDestOffset);
        Assert.Equal(64, written);
        for (int i = 0; i < 64; i++)
        {
            Assert.Equal((byte)(i * 3 + 7), dest[i]);
        }
    }
    ...
    public void SpanOverloadDoesNotLeakRentedBuffers()
    {
        ...
        Assert.Equal(1000, impl.DecompressCallsReceived);
    }
}
