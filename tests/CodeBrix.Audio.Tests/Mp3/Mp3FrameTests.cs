...
public class Mp3FrameTests
{
    ...
    public void ParsesExpectedPropertiesFromValidFrame()
    {
        using (var ms = new MemoryStream(ConstructConsecutiveValidMp3Frames(3)))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
            Assert.NotNull(frame);
            Assert.Equal(MpegVersion.Version2, frame.MpegVersion);
            Assert.Equal(MpegLayer.Layer3, frame.MpegLayer);
            Assert.False(frame.CrcPresent);
            Assert.Equal(BitRateIndex, frame.BitRateIndex);
            Assert.Equal(8000, frame.BitRate);
            Assert.Equal(22050, frame.SampleRate);
            Assert.Equal(576, frame.SampleCount);
            Assert.Equal(ValidFrameLength, frame.FrameLength);
            Assert.Equal(ChannelMode.Stereo, frame.ChannelMode);
            Assert.Equal(0, frame.ChannelExtension);
            Assert.False(frame.Copyright);
            Assert.Equal(0, frame.FileOffset);
            Assert.NotNull(frame.RawData);
            Assert.Equal(ValidFrameLength, frame.RawData.Length);
            Assert.Equal(validMp3FrameHeader[0], frame.RawData[0]);
            Assert.Equal(validMp3FrameHeader[1], frame.RawData[1]);
            Assert.Equal(validMp3FrameHeader[2], frame.RawData[2]);
            Assert.Equal(validMp3FrameHeader[3], frame.RawData[3]);
        }
    }
    ...
    public void ReadDataFalseSkipsBodyAndLeavesRawDataNull()
    {
        using (var ms = new MemoryStream(ConstructConsecutiveValidMp3Frames(1)))
        {
            ...
            Assert.NotNull(frame);
            Assert.Null(frame.RawData);
            Assert.Equal(frame.FileOffset + frame.FrameLength, ms.Position);
        }
    }
    ...
    public void RejectsFrameWithReservedMpegVersion()
    {
        ...
        {
            Assert.Null(Mp3Frame.LoadFromStream(ms));
        }
    }
    ...
    public void AcceptsFrameWithChannelExtensionInJointStereo()
    {
        ...
        using (var ms = new MemoryStream(header))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms, false);
            Assert.NotNull(frame);
            Assert.Equal(ChannelMode.JointStereo, frame.ChannelMode);
            Assert.Equal(2, frame.ChannelExtension);
        }
    }
    ...
    public void CanParseMp3FrameOffsetByN(int offset)
    {
        ...
        Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
        Assert.NotNull(frame);
        Assert.Equal(offset, frame.FileOffset);
    }
    ...
    public void SkipsId3v2TagAndFindsFirstAudioFrame()
    {
        ...
        using (var ms = new MemoryStream(data))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
            Assert.NotNull(frame);
            Assert.Equal(id3Header.Length + fakeArtworkData.Length, frame.FileOffset);
        }
    }
    ...
    public void SkipsId3v2TagWithFooterAndFindsFirstAudioFrame()
    {
        ...
        using (var ms = new MemoryStream(data))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
            Assert.NotNull(frame);
            Assert.Equal(id3Header.Length + fakeId3Payload.Length + id3Footer.Length, frame.FileOffset);
        }
    }
    ...
    public void DoesNotSkipId3HeaderWithInvalidSynchsafeSize()
    {
        ...
        using (var ms = new MemoryStream(data))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
            Assert.NotNull(frame);
            Assert.Equal(invalidId3Header.Length, frame.FileOffset);
        }
    }
    ...
    public void RejectsSingleFalsePositiveHeaderAndFindsRealFrameSequence()
    {
        ...
        using (var ms = new MemoryStream(data))
        {
            Mp3Frame frame = Mp3Frame.LoadFromStream(ms);
            Assert.NotNull(frame);
            Assert.Equal(falsePositive.Length + filler.Length, frame.FileOffset);
        }
    }
