...
/// </summary>
/// <remarks>
/// Within one test assembly the shared-output collection already keeps them apart, but the two
/// test projects run as SEPARATE PROCESSES and would otherwise play over each other - which turns
/// the motif into mush. A named mutex is the only thing that reaches across processes. It also
/// leaves a short gap on the way out so two runs in a row stay distinct rather than running
/// together.
/// </remarks>
