...
==========================================
  Path:   tests/Assets/audio/      (manifest: AUDIO-FIXTURES.txt)
          ...
          tests/Assets/abc/        (manifest: ABC-FIXTURES.txt)
  ...
  the carried-over synthesizer suite compares against. tests/Assets/abc/ holds
  three short abc tunes written for this suite; ABC-FIXTURES.txt says what each
  is for, and MAINTAINER-README.txt explains why there are only three.

  No third-party audio, no real SoundFont, no SFZ library and no third-party
  transcription is committed anywhere in this repository. The SFZ tests build
  their instruments on the fly into a temp directory per test.
