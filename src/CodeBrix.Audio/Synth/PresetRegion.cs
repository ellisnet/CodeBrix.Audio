...
public sealed class PresetRegion
{
    ...
    private readonly SoundFontInstrument instrument;
    ...
    private PresetRegion()
    {
        ...
        instrument = SoundFontInstrument.Default;
    }

    private PresetRegion(Preset preset, Zone global, Zone local, SoundFontInstrument[] instruments) : this()
    {
        ...
        var id = gs[(int)GeneratorType.SoundFontInstrument];
    ...
    internal static PresetRegion[] Create(Preset preset, Span<Zone> zones, SoundFontInstrument[] instruments)
    {
        // Is the first one the global zone?
        if (zones[0].Generators.Count == 0 || zones[0].Generators.Last().Type != GeneratorType.SoundFontInstrument)
        {
    ...
    /// </summary>
    public SoundFontInstrument SoundFontInstrument => instrument;
