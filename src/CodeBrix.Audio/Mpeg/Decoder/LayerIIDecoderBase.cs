/*
 * CodeBrix.Audio.Mpeg - A C# MPEG1/2/2.5 audio decoder
 * 
 */

using System;

namespace CodeBrix.Audio.Mpeg.Decoder; //was previously: NLayer.Decoder;
...
abstract class LayerIIDecoderBase : LayerDecoderBase
{
    ...
    int DecodeSamples(float[] ch0, float[] ch1)
    {
        ...
        if (_channels == 2 && StereoMode == CodeBrix.Audio.Mpeg.StereoMode.DownmixToMono)
        {
            for (int i = 0; i < idx; i++)
            {
                ch0[i] = (ch0[i] + ch1[i]) / 2;
