...
* Reading and writing Standard MIDI Files (`.mid`), with the General MIDI Level 1 sound set available by name (`GeneralMidiProgram`, `GeneralMidiPercussion`, `GeneralMidi.DisplayName(...)`).
* Reading ABC notation (`.abc`), the plain-text music format: one file or a whole tunebook, read to a faithful tune model — header fields, keys and modes, meters, tempos, note lengths, broken rhythm, rests, chords, ties, grace notes, tuplets, repeats with numbered endings, and multiple voices — and converted to the editable MIDI model (`AbcToMidi.Convert`), so a tune plays through any instrument here or saves as a standard MIDI file. That makes the package a complete ABC-to-MIDI file converter in its own right: `AbcReader.Read`, `AbcToMidi.Convert`, `MidiFile.Export` — three calls, a whole tunebook in one pass, with no audio device, instrument or SoundFont involved. Timing is exact: lengths and positions are carried as fractions of a whole note and rounded to ticks once, so a triplet never pushes the rest of the tune off the beat. Repeats are unrolled on conversion, voices take their own channels and instruments (including abc2midi's `%%MIDI program` and `%%MIDI channel`), and anything the notation carried that could not be honoured is reported per tune rather than thrown.
...
* Playing MIDI music that is **still being written** (`MidiStream`): a timeline a producer appends to a bar at a time, from any thread, while the player is already sounding it. Playback runs from the head while the tail is still arriving; when it catches up it waits in silence - holding its position, letting what was sounding ring out - rather than ending, and a pre-roll says how much must be buffered before it starts and before it starts again after an underrun. An event that arrives behind the head is played at once and counted, never dropped, and a rest the producer has settled is declared rather than written, so the head plays the silence instead of waiting in it. Ticks convert to moments and back through the stream's own tempo map, defined past the point the producer has reached. The same transport as any other MIDI: the same five `Load` overloads, the same hooks, speed, per-channel controls and MPE, with a duration that grows until the producer says the music is finished. Everything appended is also kept as an editable event collection, so the piece that was heard saves as a standard MIDI file or converts to an ordinary sequence for looping or an offline render.
...
* Playing a **multi-track song** on one transport (`MultiTrackPlayer`): one track per part, where a part can be a recording, a MIDI performance through a SoundFont, SFZ or Decent Sampler instrument, or **both at once** — switching between the two is a crossfade rather than a restart, because both are always rendered in step. Per-track gain, pan, mute, solo and signed offsets; a percussion rule for the zero-length notes drum parts are written with; optional level matching that makes a synthesized part sit where the recording sat in the mix and reports what the matched mix will peak at; offline rendering with no audio device, written out through any registered writer; and an export that merges every MIDI part into one General MIDI file. A Suno stems download loads straight into one (`SunoStemsLoader`) — the zip or the extracted folder, tolerant MIDI reading that reports what it could not honour rather than throwing, note counts, coverage and the notes each transcription holds so a near-empty or out-of-range part can be spotted, a measurement of how far each transcription sits from its own recording, exposed and overridable so the synthesized parts line up with the recorded ones, and one-line ways to say which instrument library a song plays through, which instrument one named part plays through, and which parts play from their transcription at all.
...
{
    InstrumentLibraryName = "ModestSynthGm",            // one name changes the whole sound
    MidiStems = SunoStemSelection                       // everything with a usable
        .EverythingBut("Vocals", "Backing Vocals"),     //   transcription but the voices
...
// Offline instead, with no audio device: measure the levels (Prepare opens the device),
// then bounce the result in whatever format the extension names.
// player.MeasureRelativeTrackLevels(48000);
// player.Volume = player.LastLevelMatch.SuggestedVolume;   // matching peaks past 1.0
// player.RenderToFile("mix.wav", new WaveFormat(48000, 16, 2));
